function i = Polynomial_Integral(D, p_1, p_2, number)
    arguments
    ...
    %takes the upper and bottom parts of the function
    f_a = f(p_1,p_1);
    f_b = f(p_2,p_2);
    
    %creates a linearly spaced vector, same thing as in first file
    X_new = linspace(p_1,p_2,number+1);
    %recording the difference between the points
    change = X_new(2)-X_new(1);

    %running a loop to add together each component
     for i = 1:number-1
          new_y = f(p_1+i*change,p_1+i*change);
          summ = summ+new_y;
 
     end

    %integral result
    i = ((p_2-p_1)/(2*number))*(f_a+2*summ+f_b);
